    ...
    if (company.type === 'marketing') {
      const updatedAgents = await Promise.all(
        company.agents.map(async (agent) => {
          const superKey = `agente:${id}:${agent.name}`;
          const superRaw = await redis.get(superKey);
          if (superRaw) {
            const superAgent = typeof superRaw === 'string' ? JSON.parse(superRaw) : superRaw;
            if (superAgent) {
              const lastRun = superAgent.lastRunAt || superAgent.lastEvaluatedAt || 0;
              const hace24h = Date.now() - 86400000;
              const status = lastRun > hace24h ? 'executing' : (superAgent.status || 'idle');
              return { ...agent, status };
            }
          }
          return agent;
